...
def second_stage_decomposition(sets, parameters, first_stage_param, previous_cut_parameters, mod_sec, ops_sec, gen_dir, count_for_files):
    ...
    # variables previous cut
    n_ini = previous_cut_parameters[0]
    e_ini = previous_cut_parameters[1]
    m_ini = previous_cut_parameters[2]
    m_hat_ini = previous_cut_parameters[3]
    accu_lambda = previous_cut_parameters[4]
    u_trans_ini = previous_cut_parameters[5]

    # with create_opl_model(model=mod, data=dat) as opl:
    mod = mod_sec
    ops = ops_sec
    name = "MobileFactories_Second_Stage_" + str(count_for_files) + ".log"
    log_file = join(gen_dir, name)
    ...
    with create_opl_model(model=mod) as opl:
        ...
        opl.set_input("_n_ini", n_ini)
        opl.set_input("_e_ini", e_ini)
        opl.set_input("_m_ini_base", m_ini)
        opl.set_input("_m_hat_ini", m_hat_ini)
        opl.set_input("_accu_lambda", accu_lambda)
        opl.set_input("_u_trans_ini", u_trans_ini)

        opl.redirect_engine_log(log_file)
        start_time = datetime.now()
        opl.run()
        run_time = datetime.now()
        second_stage_time = (run_time - start_time).total_seconds()
        ...
        check_file = os.path.join(gen_dir, "Second_Stage_" + str(count_for_files) + ".txt")
        ...
        secondstageini_dir = os.path.join(gen_dir, "secondstage_results" + str(count_for_files))
    ...
    return z_hat, s, w, w_hat, u, f, n, e, m, m_hat, u_hat, second_stage_time